...
class TenderDetailsChunker:
    ...
    def _compose_chunk_text(self, *, section: ParsedSection, content: str) -> str:
        ...
        if section.heading_path:
            label = "Heading path" if len(section.heading_path) > 1 or section.kind != "spreadsheet_row" else "Sheet context"
            parts.append(f"{label}: {' > '.join(section.heading_path)}")
        if section.title:
